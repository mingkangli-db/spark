...
private[spark] class SparkUncaughtExceptionHandler(val exitOnUncaughtException: Boolean = true)
  ...
  override def uncaughtException(thread: Thread, exception: Throwable): Unit = {
    try {
      ...
      if (!ShutdownHookManager.inShutdown()) {
        val rootCauseError = Executor.findRootCauseError(exception, killOnFatalErrorDepth)
        rootCauseError match {
          case Some(_: OutOfMemoryError) =>
            System.exit(SparkExitCode.OOM)
          case Some(e: SparkFatalException) if e.throwable.isInstanceOf[OutOfMemoryError] =>
            // SPARK-24294: This is defensive code, in case that SparkFatalException is
            // misused and uncaught.
            System.exit(SparkExitCode.OOM)
          case Some(_: KilledByTaskReaperException) if exitOnUncaughtException =>
